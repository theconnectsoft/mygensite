...
module.exports = class TunnelCluster extends EventEmitter {
  ...
  open() {
    ...
    remote.on('error', err => {
      ...
      // Connect-time failures mean the tunnel can't be established: the
      // listener is gone (ECONNREFUSED) or rejecting the handshake (e.g.
      // ECONNRESET during server restart). Surface both as ECONNREFUSED so
      // the Tunnel re-registers through the API — a socket that never
      // reached 'connect' emits no 'close'/'dead', so without this the
      // client dies silently.
      if (err.code === 'ECONNREFUSED' || !remoteConnected) {
