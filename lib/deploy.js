...
async function deploy(options) {
  const {
    ...
    files,
    owner_email,
    // No default: only send what the caller specified. On redeploy the server
    // updates just the fields present, so an implicit access=public here would
    // silently reset ACLs configured via dashboard/PATCH (new deploys default
    // to public server-side anyway).
    access,
    auth_method,
    password,
    allowed_ips,
  ...
  if (ttl != null) form.append('ttl', String(ttl)); // ttl: 0 = unlimited, must still be sent
