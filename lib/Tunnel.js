...
module.exports = class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    // Reconnect responses return admin_token/password as null — keep the
    // values from the original response so later reconnects still work.
    this.password = body.password || this.password || null;
    this.admin_token = body.admin_token || this.admin_token || null;
  ...
  _establish(info) {
    ...
    // a successful socket resets the reconnect backoff
    this.tunnelCluster.on('open', () => {
      this._retryDelay = 1000;
    });

    // connection refused means the remote listener is gone (server
    // restart, grace timeout) — reconnecting to the same port can never
    // succeed, so re-register through the API instead of giving up.
    // Other socket errors are re-emitted as before.
    this.tunnelCluster.on('error', err => {
      ...
      if (err.code === 'ECONNREFUSED') {
        ...
        // after close() this is just teardown noise from dying sockets —
        // re-emitting would crash listener-less processes
        return;
      }
      this.emit('error', err);
    });
  ...
  // full reconnect: the remote TCP listener is gone, so ask the API to
  // re-register the tunnel (passing our admin_token to reclaim the slug)
  // and open a fresh socket cluster — keeps the tunnel alive like an
  // auto-reconnecting ssh tunnel
  _reestablish() {
    if (this.closed || this._reestablishing) {
      return;
    }
    this._reestablishing = true;

    // drop the dead cluster: its remaining sockets will error/close and
    // must not trigger more reopens or reconnects — but keep swallowing
    // their 'error' events (an unhandled 'error' would throw)
    if (this.tunnelCluster) {
      this.tunnelCluster.removeAllListeners();
      this.tunnelCluster.on('error', err => {
        debug('stale cluster error ignored: %s', err.message);
      });
    }

    const delay = this._retryDelay || 1000;
    this._retryDelay = Math.min(delay * 2, 15000);
    debug('re-establishing tunnel in %dms', delay);
    ...
    const timer = setTimeout(() => {
      ...
      // reclaim our slug on re-registration
      if (this.admin_token) {
        this.opts.admin_token = this.admin_token;
      }
      this._init((err, info) => {
        this._reestablishing = false;
        if (this.closed) {
        ...
        if (err) {
          // _init retries network errors itself — an error here is a 4xx
          // from the API (slug taken over, invalid token) and won't heal
          this.emit('error', err);
          return;
        }
